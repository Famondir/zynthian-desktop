...
This repo (`Akkordeon/Zynthian`) holds the orchestration/dev-tooling layer: `docker/`, `run_zynthian_docker.sh`, `run_zynthian_vnc.sh`, `openspec/` planning docs, `.claude/` tooling. The actual Zynthian application and its native install live elsewhere, in their own separate git repos:
...
The Docker image (`docker/Dockerfile`) clones its own fresh copy of `zynthian-ui` (same fork/branch) at build time - it does **not** use `/zynthian/zynthian-ui`, which is only relevant to the native install.
