...
Provide dev-only tooling and documentation for simulating the Korg Fisa Suprema's MIDI and audio input via VMPK + `snd-aloop` on the native desktop install, for use when the physical device isn't available.
