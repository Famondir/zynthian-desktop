...
Provide automated, headless, one-command verification that the Docker desktop image boots, renders its GUI, and correctly passes MIDI through to produced audio output - using the same virtual MIDI/audio devices already validated for native dev testing, so this doesn't depend on the physical Suprema or a human watching the session.
