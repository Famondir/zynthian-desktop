...
Provide automated, headless, one-command verification that real Zynthian usage patterns work end to end - driven by a shared engine that runs the same workflow scripts against both native and Docker sessions via MIDI-injected CUIA actions, replacing the Docker-only `docker-smoke-testing` capability this supersedes.
