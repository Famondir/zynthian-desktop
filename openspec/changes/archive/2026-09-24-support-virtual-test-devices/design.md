...
- [VMPK's default ALSA client name ("MIDI Out") is generic - the whitelist match could theoretically pick up an unrelated app that happens to register the same ALSA-seq client name] → Low real-world risk (scoped to `a2j:`-prefixed bridge ports, i.e. still requires going through a2jmidid), and matches the precedent already set by the other whitelist entries (also generic-ish names like "aubio"). **Correction found during task 4.1 validation**: on the VMPK version installed this session, even a genuinely from-scratch config (no prior `~/.config/vmpk.sourceforge.net` at all) produced the ALSA client name `VMPK Output`, not `MIDI Out` - the exact condition that produces the literal `MIDI Out` name (assumed above to be "no config file yet") wasn't reproduced. Doesn't affect the fix - the regex's `VMPK` alternative covers `VMPK Output` fine - but the specific "pre-config default" theory here is unconfirmed, possibly stale (an older VMPK version's behavior) or specific to a state not reproduced.
...
- [**Found during task 4.2 validation**: `snd-aloop` cannot simulate a device disappearing *while actively in use*, the scenario that matters most for `fix-audio-hotplug-support` (the Suprema being unplugged mid-session) - `modprobe -r snd-aloop` refuses with "Module snd_aloop is in use" as long as Zynthian's own `zynain_Loopback`/`zynaout_Loopback` `alsa_in`/`alsa_out` bridge processes hold the device open, since kernel module removal respects normal reference counting. Real USB hardware doesn't have this limit - unplugging a USB audio interface forces the kernel to disconnect it regardless of open handles, generating the remove event unconditionally] → No code fix applicable here (this is a fundamental difference between a loadable kernel module and a hot-pluggable USB device, not a bug). Documented as a real scope boundary of the `snd-aloop` stand-in: it validates the *device-appears* / *device already released, then disappears* paths well, but the *abrupt in-use removal* path - arguably the single most important case for hotplug robustness - still needs the physical Suprema/AG03, same as `fix-audio-hotplug-support`'s own remaining real-hardware tasks.
