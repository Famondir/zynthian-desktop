...
- [ ] 3.4 Build the image and verify `$ZYNTHIAN_DIR/zynthian-webconf` (+ `cert/`) and its deps are present (`docker run --rm <image> ls /zynthian/zynthian-webconf/cert`, `docker run --rm <image> /zynthian/venv/bin/pip show tornado`).
...
- [ ] 4.3 Verify webconf's process comes up on container start and is cleanly killed on container stop (check `/tmp/zynthian_webconf.log`, check no orphaned process after `docker stop`).
